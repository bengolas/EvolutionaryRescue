...
rm(list=ls())
  ...
  SIR.sto <- function(g.start,sigma,t,r,mu,B,alpha,K,phi,delta,lambda,Br,S.start,E.start,I.start,R.start,M.start){
    ...
    for(i in 2:XT){
      ...
      birth <- mu*N[i-1]
      nat <- mu*N[i-1]
      infect <- B*delta*S[i-1]*I[i-1] + Br*delta*S[i-1]*M
      disease <- alpha*I[i-1]
      recover <- B*gtrait*I[i-1] + Br*gtrait*I[i-1]
      resistloss <- phi*R[i-1]
      decay <- lambda*M
      
      sumrates <- birth+nat+infect+disease+recover+resistloss+decay
      if(sumrates <= 0) break
      ...
      } else{if(u<(birth+nat+infect+disease+recover+resistloss)/sumrates){
        if(R[i-1]<=1){  
          Loser <- which(individuals==3,arr.ind=TRUE)
        } else{
          Loser=sample(which(individuals==3,arr.ind=TRUE),1)
        } #end else
        individuals[Loser] <- 1
      } else{
        M <- M-1
