  ...
  B <- 0.5       # Transmission coefficient
  ...
  SIR.sto <- function(g.start,sigma,t,r,mu,B,alpha,K,rho,S.start,I.start,R.start){
    start_time <- Sys.time()
  ...
  results <- foreach(i = 1:length(g.values), .combine= cbind) %dopar% {
    ...
    ind.results <- mapply(SIR.sto,g.start,sigma,t=t,r=r,mu=mu,B=B,alpha=alpha,K=K,rho=rho,S.start = S.start,I.start = I.start,R.start = R.start)
    return(ind.results)
  }
  ...
  write.csv(x = as.data.frame(ind.results), file = "Fig1ResultsSIRlowBc.csv")
  ...
  ggplot(group.results,aes(g.1,p.survive,group=variance)) + geom_line(aes(color=variance)) + 
    ...
    xlab("Initial trait value") + ylab("Probability of survival") +theme_minimal()
