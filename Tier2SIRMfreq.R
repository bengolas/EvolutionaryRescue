...
rm(list=ls())
  ...
  SIR.sto <- function(g.start,sigma,t,r,mu,B,alpha,K,phi,delta,lambda,Br,S.start,E.start,I.start,R.start,M.start){
    ...
    for(i in 2:XT){
      ...
      birth <- mu*N[i-1]
      nat <- mu*N[i-1]
      infect <- B*S[i-1]*I[i-1]/N[i-1] + Br*S[i-1]*M
      disease <- alpha*I[i-1]
      recover <- gtrait
      decay <- lambda*M
  ...
  sigma.values <- seq(0.005,0.2,0.005)
  
  results <- foreach(i = 1:length(sigma.values), .combine= cbind) %dopar% {
    sigma <-  rep(sigma.values[i], rep)
    g.start <- 0
    ind.results <- mapply(SIR.sto,sigma,g.start=g.start,t=t,r=r,mu=mu,B=B,alpha=alpha,K=K,phi=phi,delta=delta,lambda=lambda,Br=Br,S.start=S.start,E.start=E.start,I.start=I.start,R.start=R.start,M.start=M.start)
    return(ind.results)
  }
  ...
  write.csv(x = as.data.frame(group.results), file = "Tier2SIRMfreq_0g.csv")
...
#Visualize results
  ...
  files <- c("Tier2SIRMfreq_0g.csv","Tier2SIRMfreq.csv")
  ...
  df$Model <- gsub("Tier2","",df$Model)
  df$Model <- factor(df$Model, levels=c("SIRMfreq_0g","SIRMfreq"))  
