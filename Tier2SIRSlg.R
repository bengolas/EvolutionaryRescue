...
rm(list=ls())
  ...
  SIR.sto <- function(g.start,sigma,t,r,mu,B,alpha,K,phi,delta,S.start,E.start,I.start,R.start,M.start){
    ...
    for(i in 2:XT){
      ...
      nat <- mu*N[i-1]
      infect <- B*S[i-1]*I[i-1]
      disease <- alpha*I[i-1]
      recover <- gtrait
      resloss <- phi*R[i-1]
      
      sumrates <- birth+nat+infect+disease+recover+resloss
      if(sumrates <= 0) break
      ...
      if(u<birth/sumrates){
        ...
        if(R[i-1]<=1){  
          Suscept <- which(individuals==3,arr.ind=TRUE)
        } else{
          Suscept <- sample(which(individuals==3,arr.ind=TRUE),1)
        } #end else
        individuals[Suscept] <- 1
      } #end else 5
      } #end else 4
      } #end else 3
      } #end else 2
  ...
  write.csv(x = as.data.frame(group.results), file = "Tier2SIRSlg.csv")
