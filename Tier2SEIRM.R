  ...
  SIR.sto <- function(g.start,sigma,t,r,mu,B,alpha,K,phi,delta,lambda,S.start,E.start,I.start,R.start,M.start){
    ...
    for(i in 2:XT){
      ...
      Itrait <- which(individuals==3)
      gtrait <- sum(g[Itrait])
      
      #Potential Events
      birth <- mu*N[i-1]
      nat <- mu*N[i-1]
      expose <- B*S[i-1]*I[i-1] + Br*S[i-1]*M
      infect <- delta*E[i-1]
      disease <- alpha*I[i-1]
      recover <- gtrait
      decay <- lambda*M
      
      sumrates <- birth+nat+expose+infect+disease+recover+decay
      if(sumrates <= 0) break
      ...
      if(u<birth/sumrates){
        ...
        g <- g[-Goner]
      } else{if(u<(birth+nat+expose)/sumrates){
      ...
      } else{if(u<(birth+nat+expose+infect)/sumrates){
        if(E[i-1]<=1){  
          Infectious <- which(individuals==2,arr.ind=TRUE)
        } else{
          Infectious <- sample(which(individuals==2,arr.ind=TRUE),1)
        } #end else
        individuals[Infectious] <- 3 #Infectious
      } else{if(u<(birth+nat+expose+infect+disease)/sumrates){
        if(I[i-1]<=1){  
        ...
        g <- g[-Released]
        M <- M+1
      } else{if(u<(birth+nat+expose+infect+disease+recover)/sumrates){
        if(I[i-1]<=1){  
          EyeOfTiger <- which(individuals==3,arr.ind=TRUE)
        } else{
          EyeOfTiger=sample(which(individuals==3,arr.ind=TRUE),1,prob=g[Itrait]/gtrait)
        } #end else
      ...
      E <- cbind(E,length(which(individuals==2)))
      I <- cbind(I,length(which(individuals==3)))
      R <- cbind(R,length(which(individuals==4)))
  ...
  results <- foreach(i = 1:length(g.values), .combine= cbind) %dopar% {
    ...
    ind.results <- mapply(SIR.sto,g.start,sigma=sigma,t=t,r=r,mu=mu,B=B,alpha=alpha,K=K,phi=phi,delta=delta,lambda=lambda,S.start=S.start,E.start=E.start,I.start=I.start,R.start=R.start,M.start=M.start)
    return(ind.results)
  }
  ...
  write.csv(x = as.data.frame(group.results), file = "Tier1SEIRM.csv")
