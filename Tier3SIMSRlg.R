...
rm(list=ls())
  ...
  SIR.sto <- function(g.start,sigma,t,r,mu,B,alpha,K,phi,delta,lambda,Br,S.start,E.start,I.start,R.start,M.start){
    ...
    for(i in 2:XT){
      ...
      disease <- alpha*I[i-1]
      recover <- B*gtrait*I[i-1] + Br*gtrait*M
      decay <- lambda*M
      
      sumrates <- birth+nat+infect+disease+recover+decay
      if(sumrates <= 0) break
  ...
  write.csv(x = as.data.frame(group.results), file = "Tier3SIMSRlg.csv")
